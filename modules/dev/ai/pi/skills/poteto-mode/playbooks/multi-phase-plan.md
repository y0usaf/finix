...
3. Explore in subagents with `the Pi agent worker type` and an explicit model per the Subagents section (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
- [ ] Run `/skill:unslop` before each commit and `/skill:unslop` before review.
...
Each live lane runs on its own cloud VM at the PR head. Drive through `control-ui` or `control-cli` from `project tooling`.
