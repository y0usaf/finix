...
The parent finds its own transcript file before fanning out. The system prompt names the active workspace's `agent-transcripts/` directory; use that path. Do not glob across `unrelated project transcript directories`. That crosses workspace boundaries and reads private chats from unrelated projects.
