...
For subprocess spawning examples, surface overview, and the extended guide, read `references/detailed-guide.md` only when that material is needed. Existing templates remain available under `templates/`. Replacing or disabling a built-in tool — including why `delegate_task` cannot be overridden, and the flake-input traps when wiring a local checkout — is in that guide's "Replacing or disabling a built-in tool".
