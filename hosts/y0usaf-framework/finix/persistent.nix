...
in {
  ...
  boot = {
    ...
    initrd = {
      ...
      finit.tasks = {
        reset-home = {
          description = "rotate @home back to @home-blank";
          ...
          script = ''
            ...
            if ! btrfs subvolume show "$B/@home" >/dev/null 2>&1; then
              if btrfs subvolume show "$B/@home-new" >/dev/null 2>&1; then
                mv "$B/@home-new" "$B/@home" || exit 1
              elif btrfs subvolume show "$B/@home-lastboot" >/dev/null 2>&1; then
                mv "$B/@home-lastboot" "$B/@home" || exit 1
              else
                echo "reset-home: no recoverable @home" >&2
                exit 1
              fi
              exit 0
            fi
            if btrfs subvolume show "$B/@home-new" >/dev/null 2>&1; then
              btrfs subvolume delete "$B/@home-new" || exit 0
            fi
            if ! btrfs subvolume show "$B/@home-blank" >/dev/null 2>&1; then
              echo "reset-home: no template; preserving live home"
              exit 0
            fi
            btrfs subvolume snapshot "$B/@home-blank" "$B/@home-new" || exit 0
  ...
  users.users.${userName} = {
    ...
    passwordFile = lib.mkForce "/persist/secrets/password-hashes/${userName}";
    ...
    persistentSshAuthorizedKeys = {
      ...
      text = ''
        ...
        ${pkgs.coreutils}/bin/install -m 0644 -o root -g root \
          ...
          /persist/etc/ssh/authorized_keys.d/${userName}
      '';
    };
    persistentMachineId.text = ''
  ...
  finit = {
    tasks = {
      ...
      persist-user-binds = {
        description = "replay Framework user persistence allowlist";
        command = "${pkgs.writeShellScript "persist-user-binds" ''
          ...
          mountpoint -q /persist || exit 1
          mountpoint -q /home || exit 1

          install -d -m 0700 -o ${userName} -g users ${homeDir}
          install -d -m 0755 /persist/home/${userName}
          chown ${userName}:users ${homeDir} || true
          ...
          ensure_parents() {
            ...
            while [ -n "$rest" ]; do
              ...
              if [ -d "$directory" ]; then
                chown ${userName}:users "$directory" || true
              else
                install -d -m 0755 -o ${userName} -g users "$directory" || true
              fi
            done
          }

          failed=0
          src_root=/persist/home/${userName}
          dst_root=${homeDir}
          while IFS= read -r relative; do
            [ -n "$relative" ] || continue
            src="$src_root/$relative"
            dst="$dst_root/$relative"
            ensure_parents "$src_root" "$relative"
            ensure_parents "$dst_root" "$relative"
            [ -d "$src" ] || install -d -o ${userName} -g users "$src" || { failed=1; continue; }
            [ -d "$dst" ] || install -d -o ${userName} -g users "$dst" || { failed=1; continue; }
            mountpoint -q "$dst" || mount --bind "$src" "$dst" || failed=1
          done < ${pkgs.writeText "framework-persist-user-dirs" (lib.concatMapStrings (directory: "${dirPath directory}\n") userPersist.directories)}
          ...
          while IFS= read -r relative; do
            ...
            ensure_parents "$src_root" "$relative"
            ensure_parents "$dst_root" "$relative"
            [ -f "$src" ] || install -o ${userName} -g users -m 0600 /dev/null "$src" || { failed=1; continue; }
            [ -f "$dst" ] || install -o ${userName} -g users -m 0600 /dev/null "$dst" || { failed=1; continue; }
            mountpoint -q "$dst" || mount --bind "$src" "$dst" || failed=1
          done < ${pkgs.writeText "framework-persist-user-files" (lib.concatMapStrings (file: "${dirPath file}\n") userFiles)}
